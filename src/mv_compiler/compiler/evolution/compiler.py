...
from .ast_helpers import parse_versioned_operation, unwrap_mv_var
from .model import EntityHistory, ParsedModule
from .parser import parse_evolution_module


def transform_evolution_module(
    rel_path: Path,
    tree: ast.Module,
    *,
    ...
    parsed = parse_evolution_module(rel_path, tree)
...
def _single_definition_node(entity: EntityHistory) -> ast.AST:
    ...
    node = copy.deepcopy(version.node)
    if entity.kind == "variable":
        # `_mv.var(...)` は parser で右辺に戻してあるので、そのまま通常代入として出力できる。
        return node
    return node
...
def has_evolution_syntax(tree: ast.Module) -> bool:
    """scanner が evolution module を判定するための軽量検出。"""
    for node in tree.body:
        if isinstance(node, ast.With):
            if parse_versioned_operation(node) is not None:
                return True
        if _contains_mv_var(node):
            return True
    return False


def _contains_mv_var(node: ast.AST) -> bool:
    try:
        _, variable_versioning = unwrap_mv_var(node)
    except Exception:
        return False
    return variable_versioning is not None
