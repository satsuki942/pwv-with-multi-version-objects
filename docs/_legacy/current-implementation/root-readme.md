...
この文書は、次期仕様ドキュメントへ移行する前のルート `README.md` を退避したものである。次期仕様ではなく、現行実装参照用の旧資料として扱う。
...
詳細な入力規約と mapping 例は `versioned_modules.md` を参照してください。旧ロードマップは退避時点でリポジトリ上に残っていません。
